...
const valid = { APP_BASE_URL: 'https://redial.example.org', SUPABASE_URL: 'https://project.supabase.co', SUPABASE_PUBLISHABLE_KEY: 'sb_publishable_example_long_test_key' };
...
test('rejects local, credentialed, insecure and path-based app origins for staging', () => {
  for (const APP_BASE_URL of ['http://redial.example.org', 'https://127.0.0.1', 'https://user:secret@redial.example.org', 'https://redial.example.org/path', 'https://app.example.invalid', 'https://redial.example.org?x=1']) {
    assert.equal(checkLaunchConfig({ ...valid, APP_BASE_URL }).find(r => r.key === 'APP_BASE_URL').status, 'blocked');
  }
});
