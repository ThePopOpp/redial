...
test('Three.js journey renders, scrubs both ways and hands off to a real form', async ({ page }) => {
  ...
  const distancePerChapter = await page.locator('.call-story').evaluate(element => (element.clientHeight - innerHeight + 80) / 9);
  expect(distancePerChapter).toBeGreaterThan(2800);
  await page.mouse.wheel(0, 600);
  await expect.poll(async () => Number(await page.locator('.story-scene').getAttribute('data-progress'))).toBeGreaterThan(0);
  expect(Number(await page.locator('.story-scene').getAttribute('data-progress')) * 9).toBeLessThan(.25);
...
test('seven-step setup saves real form input, resumes, and deletes independently of the demo', async ({ page }) => {
  ...
  await expect(form).toContainText('Admin review updated');
  await form.getByRole('link', { name: 'Open my local account' }).click();
  await expect(page.getByRole('heading', { name: 'Morgan Review' })).toBeVisible();
  await expect(page.locator('.setup-details')).toContainText('Mint Mobile');
  await expect(page.locator('.setup-details')).toContainText('Warm');
  await expect(page.locator('.setup-details')).toContainText('Every call');
  await page.getByRole('link', { name: 'Admin dashboard', exact: true }).click();
  await expect(page.locator('.setup-details')).toContainText('morgan@example.test');
  await page.getByRole('combobox', { name: 'Review status', exact: true }).click();
  await page.getByRole('option', { name: 'Needs information', exact: true }).click();
  await page.getByLabel('Note for the member').fill('Please confirm your preferred connection.');
  ...
  await expect(page.locator('.setup-member-note')).toHaveText('Please confirm your preferred connection.');
  await page.getByRole('link', { name: 'Review or edit setup' }).click();
  await expect(page).toHaveURL('/#onboarding');
  await expect(form).toHaveAttribute('data-complete', 'true');
  await page.reload();
  await expect(form).toHaveAttribute('data-complete', 'true');
  await form.getByRole('button', { name: 'Delete saved details' }).click();
  await form.getByRole('button', { name: 'Delete my saved setup' }).click();
  await expect(form).toHaveAttribute('data-step', '1');
  await expect(form.getByLabel('Your name', { exact: true })).toHaveValue('');
  expect((await page.request.get('/api/onboarding/account')).status()).toBe(404);
  expect((await page.request.get('/api/onboarding/management')).status()).toBe(404);
});
