# 02 · Paste into Claude Code or Codex

Open the **existing Redial project root** in VS Code with this complete folder at `redial-build-kit/`. The current v1.1 initial prompt is reproduced below, with an entry instruction for the new ordered handoff. This is the starting task, not permission to run every feature prompt at once.

---

First read `redial-build-kit/00-START-HERE.md` and `redial-build-kit/01-BUILD-ORDER.md`. All kit-relative paths in the prompt below resolve inside `redial-build-kit/` unless the path explicitly describes files in the actual application. Treat the numbered stage guide as navigation over the existing v1.1 roadmap, and preserve existing project instructions and local changes.
