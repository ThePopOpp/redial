...
function tracingPass(position: number, start: number, peak: number, end: number, hold = .16) {
  return smooth((position - start) / (peak - start)) * (1 - smooth((position - peak - hold) / (end - peak - hold)));
}
// Trial in the first two chapters: finish presenting the phone before the card
// takes focus. The next phone reforms only after the outgoing card has left.
export function stagedReveal(position: number, chapter: number) {
  const phase = position - chapter;
  const rise = smooth((phase - .38) / .24);
...
export function timeline(progress: number, mobile = false) {
  ...
  const reveal = index < 2 ? stagedReveal(position, index) : null;
  ...
  const outline = Math.max(tracingPass(position, .08, .22, .38, .04), tracingPass(position, 3.58, 3.88, 4.32), tracingPass(position, 7.60, 7.95, 8.36), smooth((position - 8.53) / .22));
  const traceDraw = Math.max(smooth((position - .08) / .14) * (1 - smooth((position - .28) / .10)), smooth((position - 3.58) / .34) * (1 - smooth((position - 4.14) / .18)), smooth((position - 7.60) / .37) * (1 - smooth((position - 8.18) / .18)), smooth((position - 8.48) / .32));
