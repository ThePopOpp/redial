    ...
    <main id="main" tabIndex={-1}>
      ...
      <section ref={story} className="call-story" aria-label="The Redial call journey" data-chapter={chapters[frame.chapter].id} style={{ '--scene-tint': frame.tone.join(' '), '--journey-progress': position, '--journey-offset': `${frame.position * -35}px`, '--trace-opacity': frame.outline, '--travel': frame.travel } as CSSProperties}>
        <div className="story-sticky">
          ...
          {!reduced && <div className="story-feature-backdrop" aria-hidden="true"><div className="story-feature-word" data-feature="screening" data-word="SCREEN" style={{ '--word-glow': backgroundWord.glow, opacity: backgroundWord.opacity, transform: `translate3d(${backgroundWord.x}%,${backgroundWord.y}px,0) scale(${backgroundWord.scale})`, filter: `blur(${backgroundWord.blur}px)` } as CSSProperties}>SCREEN</div></div>}
