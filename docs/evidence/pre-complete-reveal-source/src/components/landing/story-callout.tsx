...
export function StoryCallout({ chapter, position, audio, choice, onChoose, stationary = false }: { chapter: number; position: number; audio: SampleAudio; choice: GuidanceId | null; onChoose: (choice: GuidanceId) => void; stationary?: boolean }) {
  ...
  if (!card) return null;
  const motion = overlayMotion(position, chapter), rich = chapter >= 1 && chapter <= 4, trial = !stationary && chapter < 2;
  ...
  return <aside className={`story-callout callout-${chapter} ${rich ? 'callout-rich' : ''} ${trial ? 'callout-trial' : ''} ${stationary ? 'callout-stationary' : ''}`} style={style} aria-labelledby={id} inert={!stationary && motion.opacity < .1} data-chapter={chapter} data-rise={motion.rise.toFixed(3)}>
    ...
    {chapter === 1 && <div className="callout-reveal-details" style={{ gridTemplateRows: `${details}fr`, opacity: details, visibility: details < .01 ? 'hidden' : 'visible' }} inert={details < .95}><div><SamplePlayer audio={audio} /><p className="sample-disclosure">Fictional voices. Enable sound, then scroll to revisit the greeting.</p></div></div>}
    {chapter === 3 && <SamplePlayer audio={audio} insider />}
