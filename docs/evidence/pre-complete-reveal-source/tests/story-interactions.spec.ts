...
test('overlay and transcript positions reverse without losing reading time', () => {
  ...
  expect(sampleTimeFor(2.5)).toBeGreaterThan(sampleTimeFor(2.1));
  expect(sampleCall.turns.at(-1)!.end).toBeCloseTo(sampleCall.duration, 2);
});

test('the first two sections finish the phone presentation before revealing the card', async ({ page }) => {
  ...
  for (const chapter of [0, 1]) {
    ...
    await expect(page.locator('.story-css-renderer')).toHaveCSS('opacity', '1');
    if (chapter === 1) await expect(page.locator('.callout-reveal-details')).toHaveAttribute('inert');
  ...
  for (const position of [2.68, 3.68, 4.68, 5.68, 6.68, 7.68]) expect(timeline(position / 9).phoneOpacity).toBe(1);
});
...
test('audio requires opt-in, plays actual local media, seeks on scroll, and stops outside the sample', async ({ page }) => {
  ...
  await scrub(page, 3.48); await expect.poll(() => audio.evaluate((el: HTMLAudioElement) => el.paused)).toBe(false);
  ...
  await scrub(page, 3.4); await expect.poll(() => audio.evaluate((el: HTMLAudioElement) => el.paused)).toBe(true);
...
test('transcript scroll and keyboard scrubbing show the matching sample turn in the card and phone', async ({ page }) => {
  ...
  await scrub(page, 2.5); await expect(transcript).toHaveAttribute('data-turn', '3');
  await expect(page.locator('.story-screen-host')).toContainText('Around ten.');
  await scrub(page, 2.05); await expect(transcript).toHaveAttribute('data-turn', '0');
...
test('all three Audible choices update the private instruction and a separate simulated conversation', async ({ page }) => {
  await page.setViewportSize({ width: 1440, height: 1000 }); await page.goto('/');
  await expect(page.locator('.story-scene')).toHaveAttribute('data-rendered', 'true'); await scrub(page, 4.28);
  ...
  await scrub(page, 4.25); await expect(choices.getByRole('button', { name: /Get a reference/ })).toHaveAttribute('aria-pressed', 'true');
});
...
test('blocked sample playback offers a retry and keeps the transcript usable', async ({ page }) => {
  ...
  await scrub(page, 2.3); await expect(page.getByRole('slider', { name: 'Sample transcript position' })).toBeVisible();
});
...
for (const theme of ['light', 'dark']) test(`interactive cards fit mobile and stay accessible in ${theme} mode`, async ({ page }) => {
  ...
  await expect(page.locator('.story-scene')).toHaveAttribute('data-rendered', 'true');
  for (const position of [.03, .68, 1.1, 1.68, 2.3, 3.3, 4.25]) {
    await scrub(page, position);
    if (position > 4) await page.getByRole('button', { name: /Check the signature/ }).click();
  ...
  for (const position of [.03, .68, 1.1, 1.68]) {
    await scrub(page, position);
    ...
    expect(trialCard.y).toBeGreaterThan(310); expect(trialCard.y + trialCard.height).toBeLessThan(642);
    await page.screenshot({ path: `test-results/staged-${theme}-360-${position}.png` });
  }
  await page.setViewportSize({ width: 360, height: 667 }); await scrub(page, 4.3);
  const shortCard = (await page.locator('.story-callout').boundingBox())!;
  const shortCopy = (await page.locator('#story-audible').boundingBox())!;
