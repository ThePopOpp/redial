...
for (const theme of ['dark', 'light']) test(`SCREEN is fully readable without masking in ${theme} mode`, async ({ page }) => {
  ...
  for (const [width, height] of [[1440, 1000], [1440, 900], [1024, 768], [768, 1024], [390, 844], [360, 667]]) {
    ...
    await expect(word).toHaveText('SCREEN'); await expect(word).toHaveCSS('filter', 'blur(0px)');
