...
export function LandingExperience() {
  ...
  return <div className={`landing-experience ${reduced ? 'is-static' : 'is-cinematic'}`} data-motion={reduced ? 'reduced' : 'full'}>
    ...
    <main id="main" tabIndex={-1}>
      <section ref={story} className="call-story" aria-label="The Redial call journey" data-chapter={chapters[frame.chapter].id}>
        <div className="story-sticky">
          <div className="story-atmosphere" aria-hidden="true"><span /><span /><div className="story-grid" /></div>
          <PhoneScene progress={progress} position={frame.position} chapter={frame.chapter} enabled={!reduced} onUnavailable={handleUnavailable} />
          ...
          <div className="story-chapters">{chapters.map((chapter, index) => {
            ...
            return <article id={`story-${chapter.id}`} key={chapter.id} className={`story-chapter chapter-${index} copy-${chapter.side}`} aria-hidden={reduced ? undefined : !active} inert={!reduced && !active} style={reduced ? undefined : { opacity, transform: `translate3d(${distance * -45}px, ${distance * -22}px, 0)`, visibility: opacity < .01 ? 'hidden' : 'visible' }}>
