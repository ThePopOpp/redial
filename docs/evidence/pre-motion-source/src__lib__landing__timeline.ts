...
const poses = [
  [2.1, -.15, 0, .08, -.32, -.085, 1],
  [-2.2, -.05, -.4, .10, .36, .065, .94],
...
export function timeline(progress: number, mobile = false) {
  ...
  return { position, chapter: Math.min(8, Math.floor(position + .13)), local, transition, pose, formReveal: smooth((position - 8.80) / .20), sceneOpacity: 1 - smooth((position - 8.94) / .06), screenOpacity: 1 - smooth((position - 8.65) / .13) };
}
