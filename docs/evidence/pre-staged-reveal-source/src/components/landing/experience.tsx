    ...
    <main id="main" tabIndex={-1}>
      ...
      <section ref={story} className="call-story" aria-label="The Redial call journey" data-chapter={chapters[frame.chapter].id} style={{ '--scene-tint': frame.tone.join(' '), '--journey-progress': position, '--journey-offset': `${frame.position * -35}px`, '--trace-opacity': frame.outline, '--travel': frame.travel } as CSSProperties}>
        <div className="story-sticky">
          ...
          {!reduced && <div className="story-feature-word" aria-hidden="true" data-feature="screening" style={{ opacity: backgroundWord.opacity, transform: `translate3d(${backgroundWord.x}%,${backgroundWord.y}px,0)`, filter: `blur(${backgroundWord.blur}px)` }}>SCREEN</div>}
          <PhoneScene progress={progress} position={frame.position} guidance={selectedGuidance?.instruction} sampleTime={audio.time} enabled={!reduced} onUnavailable={handleUnavailable} />
