...
function SamplePlayer({ audio, insider = false }: { audio: SampleAudio; insider?: boolean }) {
  return <div className="story-sample-player">
    <Button variant="outline" onClick={audio.toggle} aria-label={`${audio.playing ? 'Pause' : 'Play'} ${insider ? 'Insider' : 'screening'} audio sample`} className="sample-play-button">{audio.playing ? <Pause size={16} /> : <Play size={16} />}<span>{audio.playing ? 'Pause sample' : 'Hear the sample'}</span></Button>
...
export function StoryCallout({ chapter, position, audio, choice, onChoose, stationary = false }: { chapter: number; position: number; audio: SampleAudio; choice: GuidanceId | null; onChoose: (choice: GuidanceId) => void; stationary?: boolean }) {
  ...
  const style = { '--callout-rise': stationary ? 0 : motion.rise, '--callout-scale': stationary ? 1 : motion.scale, opacity: stationary ? 1 : motion.opacity, visibility: !stationary && motion.opacity < .02 ? 'hidden' : 'visible' } as CSSProperties;
  return <aside className={`story-callout callout-${chapter} ${rich ? 'callout-rich' : ''} ${stationary ? 'callout-stationary' : ''}`} style={style} aria-labelledby={id} inert={!stationary && motion.opacity < .1} data-chapter={chapter} data-rise={motion.rise.toFixed(3)}>
