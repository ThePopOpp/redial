...
export function createPhoneScene(mount: HTMLElement, readProgress: () => number, onLost: () => void) {
  ...
  function draw() {
    ...
    hardwareMaterials.forEach(material => { material.opacity = 1 - state.outline; material.depthWrite = state.outline < .5; }); bezel.material.opacity = .6 * (1 - state.outline);
    traceMaterial.uniforms.uDraw.value = state.traceDraw * 1.04; traceMaterial.uniforms.uOpacity.value = state.outline; traceMaterial.uniforms.uTint.value.copy(tint).lerp(white, .3); drawing.visible = state.outline > .002;
    ...
    shadow.position.x = horizontal; shadow.material.opacity = (document.documentElement.classList.contains('light') ? .30 : .6) * (1 - state.outline * .65) * (1 - state.formReveal);
    ...
    css.domElement.style.opacity = String(state.screenOpacity * (1 - smooth((state.outline - .12) / .82)));
    ...
    mount.dataset.progress = progress.toFixed(4); mount.dataset.rendered = 'true'; mount.dataset.outline = state.outline.toFixed(3); mount.dataset.trace = state.traceDraw.toFixed(3); mount.dataset.pointer = `${pointerX.toFixed(3)},${pointerY.toFixed(3)}`;
  }
  draw();
