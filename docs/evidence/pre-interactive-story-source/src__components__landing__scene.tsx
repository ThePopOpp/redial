...
export function PhoneScene({ progress, position, enabled, onUnavailable }: { progress: RefObject<number>; position: number; enabled: boolean; onUnavailable: () => void }) {
  ...
  return <div className="story-scene" ref={mount} aria-hidden="true" data-testid="three-scene">{screen && enabled && createPortal(<PhoneDisplay position={position} />, screen)}</div>;
}
