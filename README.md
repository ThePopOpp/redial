...
The project now includes a password-protected Docker deployment for Coolify, persistent preview storage, and runtime configuration checks for Supabase, Twilio, Resend or Hostinger SMTP. Deploy the `main` branch with the **Dockerfile** build pack; start with [the development deployment guide](docs/DEVELOPMENT-DEPLOYMENT.md) and [.env.example](.env.example). The deployment hostname is `redial.si`; `dev.redial.si` also resolves and is available for a separate staging application. Live customer accounts, calling and email delivery remain pending; adding provider variables does not activate them.
