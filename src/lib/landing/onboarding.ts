...
export function validateDraft(draft: OnboardingDraft) {
  return Object.assign({}, draft.complete && draft.step !== 7 ? { step: 'Finish the final review before submitting.' } : {}, ...Array.from({ length: draft.complete ? 7 : draft.step - 1 }, (_, index) => validateStep(draft, index + 1))) as Record<string, string>;
}
