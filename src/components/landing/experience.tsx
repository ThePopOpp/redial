...
import { chapters, clamp, formPreviewStart, formZoomEnd, smooth, timeline, screeningChoreography } from '@/lib/landing/timeline';
    ...
    <main id="main" tabIndex={-1}>
      ...
      <section ref={story} className="call-story" aria-label="The Redial call journey" data-chapter={chapters[frame.chapter].id} style={{ '--scene-tint': frame.tone.join(' '), '--journey-progress': position, '--journey-offset': `${frame.position * -35}px`, '--trace-opacity': frame.outline, '--travel': frame.travel, '--setup-zoom': frame.zoom } as CSSProperties}>
        <div className="story-sticky">
          {!reduced && <div className="transcript-edge-glow" aria-hidden="true" style={{ opacity: frame.transcriptGlow }} />}
          ...
          <div className="story-chapters">{chapters.map((chapter, index) => {
            const copyReveal = index === 1 ? screeningChoreography(frame.position).copy : 1; const active = frame.chapter === index; const distance = frame.position - index; const opacity = reduced ? 1 : (distance < 0 ? smooth((distance + .16) / .16) : 1 - smooth((distance - (index === 8 ? .08 : index === 1 ? .72 : .57)) / (index === 1 ? .15 : .27))) * copyReveal; const Icon = icons[index];
            return <article id={`story-${chapter.id}`} key={chapter.id} className={`story-chapter chapter-${index} copy-${chapter.side}`} aria-hidden={reduced ? undefined : !active || opacity < .01} inert={!reduced && (!active || opacity < .01)} style={reduced ? undefined : { opacity, transform: `translate3d(${distance * -24 + (1 - copyReveal) * 100}px, ${distance < 0 ? -distance * 180 : -distance * 10 - smooth((distance - (index === 1 ? .72 : .50)) / .35) * 60}px, 0)`, filter: `blur(${(1 - opacity) * 5}px)`, visibility: opacity < .01 ? 'hidden' : 'visible' }}>
