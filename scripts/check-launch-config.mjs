...
export function checkLaunchConfig(env) {
  ...
  const forbidden = Object.keys(env).filter(key => /^(NEXT_PUBLIC_.*(SECRET|SERVICE_ROLE|AUTH_TOKEN|API_KEY)|SUPABASE_(SERVICE_ROLE_KEY|SECRET_KEY)|TWILIO_AUTH_TOKEN|XAI_API_KEY|RESEND_API_KEY|SQUARE_ACCESS_TOKEN)$/.test(key) && env[key]);
